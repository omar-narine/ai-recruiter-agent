import { Calendar, CircleHelp, Clock, Tag } from "lucide-react";
    ...
    <div className="p-5 bg-white rounded-lg mt-5">
      ...
      <div className="mt-4 flex items-center justify-between lg:pr-52">
        ...
        {interviewDetail?.type && (
          <div>
            <h2 className="text-sm text-gray-500">Primary Type</h2>
            <h2 className="flex text-sm font-bold items-center gap-2">
              <Tag className="h-4 w-4" /> {JSON.parse(interviewDetail?.type)[0]}
            </h2>
          </div>
        )}
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          {interviewDetail?.questionList.map((item, index) => (
            <h2 key={index} className="mt-1 text-sm leading-6">
